...
def create_authorization_url():
    ...
    params={"client_id":settings.spotify_client_id,"response_type":"code","redirect_uri":settings.spotify_redirect_uri,"scope":SCOPES,"code_challenge_method":"S256","code_challenge":challenge,"state":state}
    return AUTHORIZE+"?"+urlencode(params),state

async def exchange_code(code:str):
    if not _verifier: raise RuntimeError("PKCE verifier missing; restart authorization")
    async with httpx.AsyncClient(timeout=20) as client:
        r=await client.post(TOKEN,data={"client_id":settings.spotify_client_id,"grant_type":"authorization_code","code":code,"redirect_uri":settings.spotify_redirect_uri,"code_verifier":_verifier}); r.raise_for_status(); token=r.json()
